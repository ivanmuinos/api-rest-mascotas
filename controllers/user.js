...
function addFavorite(req, res){
	...
	User.find({
		...
		if(favorites.length>0){
			return res.status(200).send({message: 'Ya tenes el favorito'});
		} else{
			User.findByIdAndUpdate(req.user.sub, {$push: {favorites: petId}}, {safe: true, upsert: true}, (err, userUpdated) => {
				if(err) return res.status(500).send({message: 'Error al guardar favorito'});
				if(!userUpdated) return res.status(404).send({message: 'No se pudo guardar el favorito'});
				return res.status(200).send({user: userUpdated});	
			});
		}
	});
